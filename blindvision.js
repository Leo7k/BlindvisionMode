...
(function () {
	...
	Список этих селекторов необходимо менять под структуру сайта, на котором размещается скрипт.
	*/
	...
	/*
	Селекторы, указывающие на элементы управления окном настройки.
	*/
	...
	/*
	Функции, вызываемые при нажатии кнопок. Менять не нужно.
	*/
	...
	/*
	  Таблица соответствия CSS-селекторов и включенных CSS-фильтров.
	  Формат: blindvisionCSSFilters[selector] = cssFilter
	*/
	var blindvisionCSSFilters = {};
	
	//Элементы формы, значение которых берется из .checked, а не из .value
	var BLINDVISION_INPUT_TYPES_CHECKED = {
		checkbox: true,
		radio: true
	};
	var bodyOverflowInitialStatus;
	...
	function blindvisionConfigCtrlApplyChange(element) {
	...
	//Применение настроек озвучки текста
	...
	//Закрыть окно с сохранением и применением настроек
	...
	function blindvisionConfigApply() {
		var controlelements = $(BLINDVISION_ELEMENT_IDS.CONFIG_FORM).get(0).elements;
		for (var i = 0; i < controlelements.length; i++) {
			blindvisionConfigCtrlApplyChange(controlelements[i]);
		}
	...
	//Разблокировать возможность включения озвучки с экрана, либо отобразить оповещение о ее недоступности
	...
	//Проверить наличие WebSpeech Synthesis в браузере и найти русскоязычные голосовые наборы.
	...
	//Запуск озвучки при наведении мыши или переключении фокуса (Tab)
	...
	//Обработка горячих клавиш управления озвучкой	
	//control - прервать
	//shift - приостановить/возобновить
